		...
		<view ref="elBtn"  class="elBtn margin-text-center margin-top-sm">
			<view  class="flex  p-xs margin-bottom-sm mb-sm">
				...
				<view  class="flex-sub  radius">
							<button :loading="false" :disabled="formItem.cardBoardNo==''" type="" class="cu-btn margin-tb-sm round line-red">{{cardBoardNoInfoList.length}}</button>
						</view>
						<view  class="flex-twice  radius"> 
						<button @click="preSubmitWorkNoList()" :loading="false" :disabled="formItem.cardBoardNo==''" type="" class="cu-btn block bg-green margin-tb-sm lg" > 提交</button>
						</view>
						...
						<view class="flex border-top">
							...
							<view class="flex-sub">
								<view class="cu-form-group">
									...
									<input class="cl-modifyInput" v-model="currentWorkNoItem.ap_Qty" type="number"  placeholder="本板数" name="input"></input>
								</view>
							</view>
						</view>
						<view ref="elBtn"  class="elBtn margin-text-center margin-top-sm">
	...
	export default {
		...
		watch:{
			formItem: {
			  handler(newVal,oldVal){
				this.tempcardBoardNo=newVal.cardBoardNo
			  },
			 ...
			 tempcardBoardNo: {
			   handler(newVal,oldVal){
				  console.log('=======oldVal.cardBoardNo========='+newVal,oldVal)
			 	// this.oldTempcardBoardNo =oldVal.cardBoardNo
			   },
			  immediate: true,  //刷新加载 立马触发一次handler
			  deep: true  // 可以深度检测到 person 对象的属性值的变化
			  },
			 // 直接监听formItem.cardBoardNo 失败,二次监听 卡板号是否变化
			 // tempcardBoardNo(newVal,oldVal){
				//  if(oldVal.cardBoardNo!=''){
				// 	  this.oldTempcardBoardNo =oldVal.cardBoardNo
				//  }
			 // }
		},
		...
		methods: {
			...
			getSelectDataInfo(item){
				...
				this.$store.commit('setCurrentLineItem',item)
				break;
				case 'Nline':
				...
				this.$store.commit('setCurrentClassItem',item)
				break;
				case 'Nclass':
			...
			pushDataIntoCardBoardList(res){
				...
				// 验证卡板号信息,当前不为空时
			  	cardBoardNotNull(){
					if(!this.isChangeCardNo){
						 this.checkCardBoardNo(this.formItem.cardBoardNo,'A').then(res=>{
							 this.formItem.cardBoardNo = ''
							 return
			...
			if(this.cardBoardNoInfoList && this.cardBoardNoInfoList.length>0){	 
					  ...
					  this.msgContent ='存在已扫描未保存数据,是否先提交保存?'
					  this.toShowModal=true
				}else{
