...
export default {
	...
	data() {
		return {
			currentFilterType:'',//当前查询类型：周 月 季
			errorContent: '数据加载中...',
			tableHeight: 0, //表格高度
			cWidth: '',
			cHeight: '',
			...
			dataColumns:[],//动态列原型
			     //按客户员汇总
		    dataColumnsM1: [
		                   {field: 'co_CustId', title:'客户编号', width: 100, titleAlign: 'center',columnAlign:'left'},
		                    {field: 'ct_Desc', title:'客户名称', width: 100, titleAlign: 'center',columnAlign:'left'},
		    ...
		    dataColumnsM2: [
		     ...
		     //按日期汇总 5
		    dataColumnsM3: [
			                   {field: 'co_Date', title:'订单日期', width: 136, titleAlign: 'center',columnAlign:'center'},
			                    {field: 'co_Qty', title:'数量', width: 136, titleAlign: 'center',columnAlign:'center'},
	...
	methods: {
		...
		handleFilterData(type){
		    ...
		    switch(type){
		      case 'week':
		        this.searchParams.startDate =dayjs().startOf('week')
		        this.searchParams.endDate =dayjs(Date.now()).format('YYYY-MM-DD')
		         break
		      case 'month':
		        this.searchParams.startDate =dayjs().startOf('month')
		        this.searchParams.endDate =dayjs(Date.now()).format('YYYY-MM-DD')
		        break 
		      case 'season':
